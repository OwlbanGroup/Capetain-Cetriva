...
class TestAllocateAndSpendProfits(unittest.TestCase):
    ...
    def test_allocate_and_spend_profits_success(self, mock_generate_account, mock_create_ach_payment):
        mock_generate_account.side_effect = ['111111111', '222222222', '333333333']
        mock_create_ach_payment.return_value = {'status': 'success'}

        total_amount = 10000.0
        description = 'Test profit allocation'
    ...
    @patch.object(BankingUtils, 'create_ach_payment')
    @patch.object(BankingUtils, 'generate_account')
    def test_allocate_and_spend_profits_account_generation_failure(self, mock_generate_account, mock_create_ach_payment):
        # Simulate failure to generate account number for second allocation
        mock_generate_account.side_effect = ['111111111', None, '333333333']
        mock_create_ach_payment.return_value = {'status': 'success'}

        total_amount = 10000.0
        description = 'Test profit allocation with failure'
        responses = BankingUtils.allocate_and_spend_profits(total_amount, description)
