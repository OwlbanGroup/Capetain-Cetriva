"""Manual integration script exercising BankingUtils end to end."""

from banking_utils import BankingUtils


def integration_test():
    """Run manual integration checks for account/routing operations."""
    bank_utils = BankingUtils()

    # Test account number generation
    try:
        account_number = bank_utils.generate_account(10)
        assert account_number is not None and len(account_number) == 10, (
    ...
    try:
        ...
        assert routing_number is not None and len(routing_number) > 0, (
            "Routing number retrieval failed"
        )
    ...
    if routing_number:
        try:
            ...
            assert isinstance(is_valid, bool), (
                "Routing number validation did not return a boolean"
            )
