...
class TestBankingUtilsExtended(unittest.TestCase):
    ...
    def test_spend_profits_for_oscar_with_provided_account(
        ...
        response = BankingUtils.spend_profits_for_oscar(
            amount, description, account_number
        )
        mock_create_ach_payment.assert_called_once_with(
            account_number, '021000021', amount, description
        )
    ...
    @patch.object(BankingUtils, 'create_ach_payment')
    @patch.object(BankingUtils, 'generate_account')
    def test_spend_profits_for_oscar_without_provided_account(
        self, mock_generate_account, mock_create_ach_payment
    ):
        ...
        mock_create_ach_payment.assert_called_once_with(
            '987654321', '021000021', amount, description
        )
        self.assertEqual(
